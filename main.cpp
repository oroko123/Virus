...
using Poststamp_Store_Iter =
std::multiset<Poststamp, Poststamp_Compare>::iterator;
...
namespace {
	...
	void print_poststamp(const Poststamp &poststamp) {
		std::cout << std::get<0>(poststamp) << " " << std::get<1>(poststamp) << " "
		<< std::get<4>(poststamp) << " " << std::get<3>(poststamp) << "\n";
	}
	...
	void query_poststamps(const Poststamp_Store &store, int yearFrom, int yearTo) {
		...
		result.sort();
		for (std::list<Poststamp>::iterator it = result.begin(); it != result.end();
			it++) {
			print_poststamp(*it);
	}
}
...
bool process_query(const Poststamp_Store &store, const std::string &line) {
	...
	if (from_date < MAX_YEAR && to_date < MAX_YEAR && from_date > MIN_YEAR &&
		to_date > MIN_YEAR && from_date <= to_date) {
...
bool process_stamp(Poststamp_Store &store, const std::string &line) {
	...
	if (valid_date && valid_price && valid_post && valid_stamp) {
		Poststamp stamp =
		build_poststamp(converted_date, converted_price, parsed_post_name,
			parsed_stamp_name, parsed_price);
		store.insert(stamp);
	} else {
		Poststamp stamp =
		build_poststamp(converted_date, converted_price, parsed_post_name,
